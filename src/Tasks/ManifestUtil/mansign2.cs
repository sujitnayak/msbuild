...
namespace System.Deployment.Internal.CodeSigning
{
    ...
    internal class ManifestUtilities
    {
        ...
        private const string Sha384SignatureMethodUri = @"http://www.w3.org/2000/09/xmldsig#rsa-sha384";
        private const string Sha512SignatureMethodUri = @"http://www.w3.org/2000/09/xmldsig#rsa-sha512";
        private const string Sha256DigestMethod = @"http://www.w3.org/2000/09/xmldsig#sha256";
        private const string Sha384DigestMethod = @"http://www.w3.org/2000/09/xmldsig#sha384";
        private const string Sha512DigestMethod = @"http://www.w3.org/2000/09/xmldsig#sha512";
        
        internal enum SupportedHashAlgorithm { SHA1, SHA256, SHA384, SHA512, INVALID };
        ...
        internal static string HashAlgorithmToString(SupportedHashAlgorithm algID)
        {
            if (SupportedHashAlgorithm.SHA1 == algID) { return "sha1"; }
            if (SupportedHashAlgorithm.SHA256 == algID) { return "sha256"; }
            if (SupportedHashAlgorithm.SHA384 == algID) { return "sha384"; }
            if (SupportedHashAlgorithm.SHA512 == algID) { return "sha512"; }
            return "INVALID";
        }

        internal static string GetCertHashAlgorithmUri(X509Certificate2 cert)
        {
            Oid oid = cert.SignatureAlgorithm;
            if (string.Equals(oid.FriendlyName, "sha1RSA", StringComparison.OrdinalIgnoreCase)) { return Sha1SignatureMethodUri; }
        ...
        internal static SupportedHashAlgorithm GetCertHashAlgorithm(X509Certificate2 cert)
        {
            Oid oid = cert.SignatureAlgorithm;
            if (string.Equals(oid.FriendlyName, "sha1RSA", StringComparison.OrdinalIgnoreCase)) { return SupportedHashAlgorithm.SHA1; }
            if (string.Equals(oid.FriendlyName, "sha256RSA", StringComparison.OrdinalIgnoreCase)) { return SupportedHashAlgorithm.SHA256; }
            if (string.Equals(oid.FriendlyName, "sha384RSA", StringComparison.OrdinalIgnoreCase)) { return SupportedHashAlgorithm.SHA384; }
            if (string.Equals(oid.FriendlyName, "sha512RSA", StringComparison.OrdinalIgnoreCase)) { return SupportedHashAlgorithm.SHA512; }
...
#pragma warning restore SYSLIB0021 // Type or member is obsolete
    ...
    internal class SignedCmiManifest2
    {
        ...
        private static byte[] ComputeHashFromManifest(XmlDocument manifestDom, bool useSha256OrHigher, X509Certificate2 cert)
        {
            ...
            {
                Oid oid = cert.SignatureAlgorithm;
                ManifestUtilities.SupportedHashAlgorithm hashAlgorithm = ManifestUtilities.GetCertHashAlgorithm(cert);
                string hashAlgorithmString = ManifestUtilities.HashAlgorithmToString(hashAlgorithm);
                if (ManifestUtilities.SupportedHashAlgorithmsFactories.TryGetValue(hashAlgorithmString, out var algorithmFactory))
                {
        ...
        private static void AuthenticodeSignLicenseDom(XmlDocument licenseDom, CmiManifestSigner2 signer, string timeStampUrl, bool useSha256OrHigher, bool disallowMansignTimestampFallback)
        {
            ...
            {
                signedXml.SignedInfo.SignatureMethod = ManifestUtilities.GetCertHashAlgorithmUri(signer.Certificate);
            }
            else
            {
            ...
            if (signer.UseSha256OrHigher)
            {
                reference.DigestMethod = ManifestUtilities.GetDigestAlgorithmUri(signer.Certificate);
            }
            else
            {
        ...
        private static void StrongNameSignManifestDom(XmlDocument manifestDom, XmlDocument licenseDom, CmiManifestSigner2 signer, bool useSha256OrHigher)
        {
            ...
            if (signer.UseSha256OrHigher)
            {
                enveloped.DigestMethod = ManifestUtilities.GetDigestAlgorithmUri(signer.Certificate);
            }
            else
            {
