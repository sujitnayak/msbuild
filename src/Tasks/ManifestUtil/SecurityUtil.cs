...
namespace Microsoft.Build.Tasks.Deployment.ManifestUtilities
{
    ...
    {
        public static string HashAlgorithmToString(Oid oid)
        {
            if (string.Equals(oid.FriendlyName, "sha1RSA", StringComparison.OrdinalIgnoreCase)) { return "sha1"; }
            ...
            return "sha256"; // Default to SHA256 for unknown algorithms
        }
...
#pragma warning restore CA2000 // Dispose objects before losing scope
        ...
        internal static string GetCommandLineParameters(X509Certificate2 cert, Uri timestampUrl, string path,
                                                        bool useSha256OrHigher, bool useRFC3161Timestamp)
        {
            string algo = HashAlgorithmToString(cert.SignatureAlgorithm);
            var commandLine = new StringBuilder();
            if (useSha256OrHigher)
            {
                commandLine.AppendFormat(CultureInfo.InvariantCulture, "sign /fd {0} /sha1 {1} ", algo, cert.Thumbprint);
            }
            else
            {
